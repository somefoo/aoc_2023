...
with open('input.txt') as f:
    ...
    def get_neighbours(node):
        ...
        if x_count == 0:
            ...
            # Super lazy solution
            skip = 3
            for i in range(min(x+min_line_jump, width), min(x+max_line_jump,width)):
                if (x,y) == (i,y): continue
                node = get_node(i,y)
                node[0] += jump_cost
                jump_cost = node[0]
            ...
            for i in range(max(0, x-min_line_jump), max(-1,x-max_line_jump), -1):
                if (x,y) == (i,y): continue
                node = get_node(i,y)
                node[0] += jump_cost
                jump_cost = node[0]
        ...
        if y_count == 0:
            ...
            skip = 3
            for j in range(min(y+min_line_jump, height), min(y+max_line_jump,height)):
                if (x,y) == (x,j): continue
                node = get_node(x,j)
                node[0] += jump_cost
                jump_cost = node[0]
            ...
            for j in range(max(0, y-min_line_jump), max(-1,y-max_line_jump), -1):
                if (x,y) == (x,j): continue
                node = get_node(x,j)
                node[0] += jump_cost
                jump_cost = node[0]
